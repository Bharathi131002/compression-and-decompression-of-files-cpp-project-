...
void huffmancode::traverse(Node* r, string str) {
...
int huffmancode::binToDec(string inStr) {
    int res = 0;
    for (auto c : inStr) {
        res = res * 2 + c - '0';
    }
...
string huffmancode::decToBin(int inNum) {
    string temp = "", res = "";
    while (inNum > 0) {
        temp += (inNum % 2 + '0');
        inNum /= 2;
...
void huffmancode::buildTree(char a_code, string& path) {
    Node* curr = root;
...
void huffmancode::createMinHeap() {
    char id;
...
void huffmancode::saveDecodedFile() {
